...
function ResourceCard({ resource, variant = "standard" }: { resource: Resource; variant?: "standard" | "local" | "featured" }) {
  ...
  if (resource.internal && resource.href) return <NavLink className={className} to={resource.href}>{content}</NavLink>;
  ...
  return (
    <section className="container resources-page" aria-labelledby="resources-heading">
      <header className="resources-hero">
        ...
        <h1 className="resources-title" id="resources-heading">{hero.title}</h1>
        <p className="resources-lede">{hero.lede}</p>
      </header>

      <section className="resources-section resources-section--local" aria-labelledby="local-heading">
        ...
        <div className="resources-grid resources-grid--local">
          {sections.local.items.map((resource) => <ResourceCard key={resource.title} resource={resource} variant="local" />)}
        </div>
      </section>
      ...
      <section className="resources-section" aria-labelledby="core-heading">
        ...
        <div className="resources-feature-grid">
          {sections.core.items.map((resource) => <ResourceCard key={resource.title} resource={resource} variant="featured" />)}
        </div>
      </section>
      ...
      <section className="resources-section" aria-labelledby="practice-heading">
        ...
        <div className="resources-list-grid">
          {sections.practice.items.map((resource) => <ResourceCard key={resource.title} resource={resource} />)}
        </div>
      </section>
    </section>
  );
